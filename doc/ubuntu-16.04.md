...
2. Run below commands to do the build.
   ...
   ```bash
   sudo docker pull zhijzhao/raspberrypi
   ```

   > Below `<>` part needs to be replaced with your own value.

   ```bash
   sudo docker run --rm -v </home/user-name/some-path/iot-hub-c-raspberrypi-docker/samples>:/repo -it zhijzhao/raspberrypi /build.sh --source blink
   ```
